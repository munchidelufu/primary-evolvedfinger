...
def fit(
    ...
    loss_fn = CrossEntropyLoss()
    #
    save_dir = f"./model/irrelevant_train/{args.model_name}/irrelevant_train_{index}/"
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_name", type=str, choices=["eegconformer", "deepnet", "shallownet"]
    )
    ...
    train_dataset, _ = data_loader.load_split_dataset(
        model_name=args.model_name, label=args.label
    )
    #
    kfold = KFoldGroupbyTrial(
        n_splits=5, shuffle=True, split_path=f"./data/DEAP/{args.model_name}/train"
    )
    #
    for i in range(5):
        #
